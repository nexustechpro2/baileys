...
export const processSyncAction = (syncAction, ev, me, initialSyncOpts, logger) => {
    ...
    if (action?.muteAction) {
        ...
        ev.emit('chats.update', [{ id, name: action.aiThreadRenameAction.newTitle || undefined, conditional: getChatUpdateConditional(id, undefined) }])
    } else if (action?.threadPinAction) {
        ev.emit('chats.update', [{ id, pinned: action.threadPinAction.pinned ? toNumber(action.timestamp) : null, conditional: getChatUpdateConditional(id, undefined) }])
    } else if (action?.newsletterSavedInterestsAction) {
        ev.emit('settings.update', { setting: 'newsletterSavedInterests', value: action.newsletterSavedInterestsAction.newsletterSavedInterests })
    } else if (action?.interactiveMessageAction) {
        ev.emit('messages.update', [{ key: { remoteJid: id, id: msgId, fromMe: fromMe === '1' }, update: { interactiveMessageAction: action.interactiveMessageAction } }])
    } else if (action?.deviceCapabilities || action?.deviceCapabilitiesV2) {
        ev.emit('creds.update', { deviceCapabilities: action.deviceCapabilities || action.deviceCapabilitiesV2 })
    } else if (action?.nctSaltSyncAction) {
        ev.emit('creds.update', { nctSalt: action.nctSaltSyncAction.salt })
    } else if (action?.bubbleLockMessageAction) {
        ev.emit('messages.update', [{ key: { remoteJid: id, id: msgId, fromMe: fromMe === '1' }, update: { bubbleLocked: !!action.bubbleLockMessageAction.locked } }])
    } else {
