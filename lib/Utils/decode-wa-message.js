...
export const decryptMessageNode = (stanza, meId, meLid, repository, logger, assertSessions) => {
      ...
      if (Array.isArray(stanza.content)) {
        for (const { tag, attrs, content } of stanza.content) {
          ...
          try {
            ...
            switch (e2eType) {
              ...
              case "msg":
                try {
                  ...
                  if (errMsg.includes('SessionNotFound') && e2eType === 'msg' && assertSessions) {
                    try {
                      logger?.debug?.({ jid: decryptionJid }, '[Signal] SessionNotFound — fetching fresh session and retrying')
                      await assertSessions([decryptionJid], true)
                      msgBuffer = await repository.decryptMessage({ jid: decryptionJid, alternateJid: decryptionAltJid, type: e2eType, ciphertext: content })
                    } catch (retryErr) {
                      const cleanError = new Error(retryErr?.message || retryErr?.toString() || 'Decryption failed')
                      cleanError.stack = retryErr?.stack
                      throw cleanError
                    }
                  } else if (errMsg.includes('InvalidMessage') && e2eType === 'msg') {
                    logger?.debug?.({ jid: decryptionJid }, '[Signal] InvalidMessage — stale session, deleting and requesting retry')
                    try {
                      const toDelete = [decryptionJid]
                      if (decryptionAltJid && decryptionAltJid !== decryptionJid) toDelete.push(decryptionAltJid)
            ...
            const isExpectedDecryptErr = errStr.includes("InvalidPreKeyId") || errStr.includes("SessionNotFound") || errStr.includes("InvalidMessage") || errStr.includes("no sender key state")
