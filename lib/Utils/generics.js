...
export const fetchLatestWaWebVersion = async (options = {}) => {
  try {
    ...
    if (!match?.[1]) return { version: getWAVersion(), isLatest: false, error: { message: 'Could not find client revision in the fetched content' } }
    const clientRevision = match[1]
    return { version: [2, 3000, +clientRevision], isLatest: true }
  } catch (error) {
    return { version: getWAVersion(), isLatest: false, error }
  }
}
