...
import { WAMessageStatus, WAProto, MESSAGE_FIELD_KEYS } from '../Types/index.js'
...
export const generateWAMessageContent = async (message, options = {}) => {
    ...
    const WRAPPERS = new Set(['viewOnceMessage', 'viewOnceMessageV2', 'ephemeralMessage', 'documentWithCaptionMessage'])
    const isWrapperMessage = messageKeys.some(k => WRAPPERS.has(k))
    if (isWrapperMessage) return WAProto.Message.create(message)

    const isRawProtoMessage = messageKeys.some(k =>
        MESSAGE_FIELD_KEYS.has(k) &&
        typeof message[k] === 'object' && message[k] !== null &&
        !HIGH_LEVEL_KEYS.includes(k) &&
        !WRAPPERS.has(k)
    )
    const hasKnownKey = messageKeys.some(k =>
        HIGH_LEVEL_KEYS.includes(k) ||
        k === 'extendedTextMessage' ||
        k === 'interactiveMessage' ||
        k === 'caption' ||
        k === 'contextInfo'
    )
    if (isRawProtoMessage && messageKeys.length === 1) return WAProto.Message.create(message)
    if (isRawProtoMessage && !hasKnownKey) return WAProto.Message.create(message)
    if (!hasKnownKey && !isRawProtoMessage) return WAProto.Message.create(message)
    ...
    if ('text' in message && !('buttons' in message) && !('templateButtons' in message) && !('sections' in message) && !('interactiveButtons' in message) && !('shop' in message)) {
        ...
        if (special) {
            ...
            m = { interactiveMessage: message.interactiveMessage }  // plain object, no create()
