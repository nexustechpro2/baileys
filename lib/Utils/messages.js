...
const buildVf = (type) => {
    const base = 'scale=512:512:force_original_aspect_ratio=increase,crop=512:512:(iw-512)/2:(ih-512)/2,fps=15,format=rgba'
...
const compressWebp = async (buf, { animated, quality = 80, stickerType = StickerTypes.ROUNDED, packName, authorName } = {}) => {
    ...
    if (detected && videoExts.has(detected.ext)) {
        ...
        const tmpIn = path.join(tmpdir(), `stk_c_in_${cuid}.${detected.ext}`)
        const tmpOut = path.join(tmpdir(), `stk_c_out_${cuid}.webp`)
        try {
            writeFileSync(tmpIn, buf)
            await ffmpegWebp([
                '-y', '-i', tmpIn,
                '-vf', buildVf(stickerType),
                '-vcodec', 'libwebp_anim', '-lossless', '0', '-compression_level', '4',
                '-q:v', String(quality), '-loop', '0', '-preset', 'default', '-an', tmpOut,
            ])
            const converted = readFileSync(tmpOut)
            return packName
                ? { buffer: await tagExif(await shrinkToLimit(converted, { animated: true }), packName, authorName), isAnimated: true }
                : { buffer: converted, isAnimated: true }
        } finally {
    ...
    if (detected?.mime === 'image/webp') {
        ...
        const isAnimated = animated ?? ((meta.pages || 1) > 1)

        if (isAnimated) {
            const out = await sharp(buf, { animated: true }).webp({ quality, effort: 1, loop: 0 }).toBuffer()
            return packName
                ? { buffer: await tagExif(await shrinkToLimit(out, { animated: true }), packName, authorName), isAnimated: true }
                : { buffer: out, isAnimated: true }
        }

        const cuid = generateMessageIDV2()
        const tmpIn = path.join(tmpdir(), `stk_c_in_${cuid}.webp`)
        const tmpOut = path.join(tmpdir(), `stk_c_out_${cuid}.webp`)
        try {
            writeFileSync(tmpIn, buf)
            await ffmpegWebp(['-y', '-i', tmpIn, '-vcodec', 'libwebp', '-lossless', '0', '-compression_level', '6', '-q:v', String(quality), '-loop', '0', '-preset', 'picture', '-an', tmpOut])
            const out = readFileSync(tmpOut)
            return packName
                ? { buffer: await tagExif(await shrinkToLimit(out, { animated: false }), packName, authorName), isAnimated: false }
                : { buffer: out, isAnimated: false }
        } finally {
            fs.unlink(tmpIn).catch(() => { })
            fs.unlink(tmpOut).catch(() => { })
        }
    ...
    // static image — wa-sticker-formatter handles jpeg/png/etc
    const converted = await new Sticker(buf, { pack: packName, author: authorName, type: stickerType, quality }).toBuffer()
    const shrunk = converted.length <= STICKER_MAX_BYTES ? converted : await shrinkToLimit(converted, { animated: false })
    return { buffer: shrunk, isAnimated: false }
}
...
const shrinkToLimit = async (buf, { animated } = {}) => {
    ...
    return [out, out2].reduce((a, b) => a.length < b.length ? a : b)
}
...
export const prepareWAMessageMedia = async (message, options) => {
    ...
    if (mediaType === 'sticker') {
        try {
            ...
            const { buffer, isAnimated } = await compressWebp(rawBuf, { packName, authorName, stickerType, quality })
            uploadData.media = buffer
            uploadData.isAnimated = isAnimated
...
export const prepareStickerPackMessage = async (stickerPack, options) => {
    ...
    const processSticker = async (s, i) => {
        ...
        const tmpIn = path.join(tmpdir(), `stk_in_${i}_${uid}`)
