...
const buildVf = (type) => {
    const base = 'scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=black@0,fps=15,format=rgba'
    if (type === StickerTypes.CIRCLE) {
        return `${base},geq=lum='p(X\\,Y)':cb='cb(X\\,Y)':cr='cr(X\\,Y)':a='clip(256-hypot(X-W/2\\,Y-H/2)\\,0\\,1)*255'`
    }
    if (type === StickerTypes.ROUNDED) {
        const a = `st(0\\,abs(X-W/2)-W/2+50);st(1\\,abs(Y-H/2)-H/2+50);st(2\\,hypot(max(ld(0)\\,0)\\,max(ld(1)\\,0))+min(max(ld(0)\\,ld(1))\\,0)-50);clip(-ld(2)\\,0\\,1)*255`
        return `${base},geq=lum='p(X\\,Y)':cb='cb(X\\,Y)':cr='cr(X\\,Y)':a='${a}'`
    }
    if (type === StickerTypes.CROPPED) {
        return 'scale=512:512:force_original_aspect_ratio=increase,crop=512:512:(iw-512)/2:(ih-512)/2,fps=15,format=rgba'
    }
    return base
}
...
const ffmpegWebp = (args) => new Promise((resolve, reject) =>
    execFile(ffmpegStatic, args, { timeout: 120_000 }, (err) => err ? reject(err) : resolve())
)

const compressWebp = async (buf, { animated, quality = 80 } = {}) => {
    const tmpIn = path.join(tmpdir(), `stk_c_in_${Date.now()}.webp`)
    const tmpOut = path.join(tmpdir(), `stk_c_out_${Date.now()}.webp`)
    try {
        writeFileSync(tmpIn, buf)
        await ffmpegWebp(['-y', '-i', tmpIn, '-vcodec', animated ? 'libwebp_anim' : 'libwebp',
            '-lossless', '0', '-compression_level', '6', '-q:v', String(quality),
...
const shrinkToLimit = async (buf, { animated } = {}) => {
    ...
    for (let q = 60; q >= 20; q -= 10) {
        const out = await compressWebp(buf, { animated, quality: q })
...
export const prepareWAMessageMedia = async (message, options) => {
    ...
    if (mediaType === 'sticker') {
        try {
            ...
            const authorName = message.author || message.publisher || message.packPublisher || options?.author || options?.publisher || options?.packPublisher || 'Nexus'
            ...
            if (isWebp) {
                ...
                if (meta.exif?.length) { uploadData.media = rawBuf }
                else {
                    const compressed = await compressWebp(rawBuf, { animated: isAnimated, quality: 80 })
                    uploadData.media = await tagExif(await shrinkToLimit(compressed, { animated: isAnimated }), packName, authorName)
                }
            } else {
                uploadData.media = await new Sticker(rawBuf, { pack: packName, author: authorName, type: stickerType, quality: message.quality || options?.quality || 80 }).toBuffer()
            }
...
const tgsToWebp = async (tgsBuffer, { quality = 80, fps = 30 } = {}) => {
    ...
    try {
        for (let i = 0; i < totalFrames; i++) {
            ...
            writeFileSync(path.join(tgsTmpDir, `f${String(i).padStart(4, '0')}.png`), png)
        }
        ...
        execFileSync(ffmpegStatic, [
            ...
            '-i', path.join(tgsTmpDir, 'f%04d.png'),
...
export const prepareStickerPackMessage = async (stickerPack, options) => {
    ...
    const authorName = publisher || packPublisher || author || options?.packPublisher || options?.publisher || options?.author || 'Nexus'
    ...
    const processSticker = async (s, i) => {
        const tmpIn = path.join(tmpdir(), `stk_in_${i}_${Date.now()}`)
        const tmpOut = path.join(tmpdir(), `stk_out_${i}_${Date.now()}.webp`)
        try {
            ...
            if (s.isLottie) {
                ...
                const tmpRaw = path.join(tmpdir(), `stk_raw_${i}_${Date.now()}.webp`)
                writeFileSync(tmpIn, buf)
                try {
                    await ffmpegWebp(['-y', '-vcodec', 'vp9', '-i', tmpIn, '-vf', buildVf(stickerType),
                        '-vcodec', 'libwebp_anim', '-lossless', '0', '-compression_level', '4',
                        '-q:v', '80', '-loop', '0', '-preset', 'default', '-an', tmpRaw])
                ...
                const raw2 = await new Sticker(buf, { pack: packName, author: authorName, type: stickerType, quality: 100 }).toBuffer()
    ...
    const processBatch = async (batch, batchIdx, totalBatches, coverFilePath) => {
        ...
        for (const key of Object.keys(batchData)) delete batchData[key]
        ...
        let thumbRes = null
        try {
            const thumbBuf = await sharp(coverFilePath).resize(252, 252).jpeg().toBuffer()
    ...
    const coverTmpPath = path.join(tmpdir(), `stk_cover_${Date.now()}.webp`)
    let coverFilePath
    try {
        if (cover) {
            ...
            writeFileSync(coverTmpPath, isWebp ? coverBuf : await new Sticker(coverBuf, { pack: packName, author: authorName, type: StickerTypes.ROUNDED, quality: 100 }).toBuffer())
        ...
        const allResults = await Promise.all(sizeBatches.map((batch, idx) => processBatch(batch, idx, totalBatches, coverFilePath)))
        for (const s of allProcessed) fs.unlink(s.filePath).catch(() => { })
