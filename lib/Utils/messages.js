...
import { osTmpdir } from './messages-media.js'
        ...
        try {
            ...
            if (isWebp) {
                ...
                if (hasExif) {
                    ...
                    const inPath = path.join(osTmpdir(), `stk_single_${Date.now()}.webp`)
                    const outPath = path.join(osTmpdir(), `stk_single_out_${Date.now()}.webp`)
                    try {
