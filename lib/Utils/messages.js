        ...
        try {
            ...
            if (isWebp) {
                ...
                if (hasExif) {
                    ...
                    const inPath = path.join(tmpdir(), `stk_single_${Date.now()}.webp`)
                    const outPath = path.join(tmpdir(), `stk_single_out_${Date.now()}.webp`)
                    try {
                        ...
                        await new Promise((resolve, reject) =>
                            execFile(ffmpegStatic, [
                                ...
                                '-vcodec', isAnimated ? 'libwebp_anim' : 'libwebp',
                                '-lossless', '0', '-compression_level', '6',
                                '-q:v', '80', '-loop', '0', '-preset', 'default', '-an', outPath,
                            ], { timeout: 120_000 }, (err) => err ? reject(err) : resolve())
                        )
                        const { Exif } = require('wa-sticker-formatter')
                        uploadData.media = await new Exif({ pack: packName, author: authorName }).add(readFileSync(outPath))
                    } finally {
...
const tgsToWebp = async (tgsBuffer, { quality = 80, fps = 30 } = {}) => {
    ...
    const tmpdir = mkdtempSync(path.join(tmpdir(), 'tgs_'))
    try {
...
export const prepareStickerPackMessage = async (stickerPack, options) => {
    // Normalize input: raw array, numeric-keyed object, or { stickers: [] }
    ...
    const processSingleSticker = async (s, i) => {
        try {
            ...
            if (s.isLottie) {
                ...
                const inPath = path.join(tmpdir(), `stk_${i}_${Date.now()}.webm`)
                const outPath = path.join(tmpdir(), `stk_${i}_${Date.now()}.webp`)
                try {
                    writeFileSync(inPath, buf)
                    await new Promise((resolve, reject) =>
                        execFile(ffmpegStatic, [
                            '-y', '-vcodec', 'vp9', '-i', inPath,
                            '-vf', buildVf(stickerType),
                            '-vcodec', 'libwebp_anim', '-lossless', '0', '-compression_level', '6',
                            '-q:v', '80', '-loop', '0', '-preset', 'default', '-an', outPath,
                        ], { timeout: 120_000 }, (err) => err ? reject(err) : resolve())
                    )
                    const { Exif } = require('wa-sticker-formatter')
                    webpBuffer = await new Exif({ pack: packName, author: authorName }).add(readFileSync(outPath))
                } finally {
                    fs.unlink(inPath).catch(() => { })
                    fs.unlink(outPath).catch(() => { })
                }
    ...
    validStickers.push(...(await Promise.all(stickers.map((s, i) => runWithLimit(() => processSingleSticker(s, i))))).filter(Boolean))
    ...
    const coverWebpBuffer = coverIsWebp
        ? coverBuf
        : await new Sticker(coverBuf, { pack: packName, author: authorName, type: StickerTypes.ROUNDED, quality: 100 }).toBuffer()
