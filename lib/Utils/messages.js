...
const CIRCLE_LAVFI = `color=black:512x512,format=rgba,geq=r=0:g=0:b=0:a='clip(256-hypot(X-W/2\\,Y-H/2)\\,0\\,1)*255'`
const ROUNDED_LAVFI = `color=black:512x512,format=rgba,geq=r=0:g=0:b=0:a='${ROUNDED_A}'`
...
const buildArgs = async (stickerType, inPath, outPath, extraIn = [], codecArgs = []) => {
    ...
    if (isShape) {
        const lavfi = stickerType === StickerTypes.CIRCLE ? CIRCLE_LAVFI : ROUNDED_LAVFI
        return ['-y', '-threads', '0', ...extraIn, '-i', inPath,
            '-f', 'lavfi', '-t', '10', '-i', lavfi,
            '-filter_complex', `[0:v]${BASE_CROP_VF},format=rgba[vid];[1:v]alphaextract[mask];[vid][mask]alphamerge[out]`,
            '-map', '[out]', ...codecArgs, '-an', outPath]
    }
    const vf = stickerType === StickerTypes.FULL ? BASE_FULL_VF : stickerType === StickerTypes.DEFAULT ? null : BASE_CROP_VF
    return ['-y', '-threads', '0', ...extraIn, '-i', inPath,
    ...
    const processSticker = async (s, i) => {
        ...
        try {
            ...
            if (s.isLottie) {
                ...
                writeFileSync(tmpIn, buf)
                try {
                    await ffmpegRun(await buildArgs(stickerType, tmpIn, tmpRaw, ['-t', '10'], ['-vcodec', 'vp9', ...WEBP_ANIM_CODEC]))
