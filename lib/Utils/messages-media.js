...
const convertToOpusBuffer = async (buffer, logger) => {
    ...
    const inputPath = _trackTmp(join(tmpdir(), 'opus-in-' + generateMessageIDV2() + inputExt))
    const outputPath = _trackTmp(join(tmpdir(), 'opus-out-' + generateMessageIDV2() + '.ogg'))
    ...
    try {
        ...
        try { await fs.unlink(inputPath) } catch { } finally { _untrackTmp(inputPath) }
        try { await fs.unlink(outputPath) } catch { } finally { _untrackTmp(outputPath) }
    }
}
...
const convertToMp4Buffer = async (buffer, logger) => {
    ...
    const detected = await fileTypeFromBuffer(buffer)
    const inputExt = detected?.ext ? `.${detected.ext}` : ''
    const inputPath = _trackTmp(join(tmpdir(), 'mp4-in-' + generateMessageIDV2() + inputExt))
    const outputPath = _trackTmp(join(tmpdir(), 'mp4-out-' + generateMessageIDV2() + '.mp4'))
    await fs.writeFile(inputPath, buffer)
    try {
        await new Promise((resolve, reject) => {
