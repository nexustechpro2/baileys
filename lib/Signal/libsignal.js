...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
  ...
  return {
    ...
    async deleteSession(jids) {
      ...
      return txn(async () => {
        const data = await migrateIndexKey(parsedKeys, 'session')
        const sessionBatch = data || {}
        for (const jid of jids) delete sessionBatch[jidToAddr(jid).toString()]
        await parsedKeys.set({ session: { 'index': sessionBatch } })
      }, `del-${jids.length}`)
    },
    ...
    async migrateSession(fromJid, toJid) {
      ...
      const batchData = await migrateIndexKey(parsedKeys, 'device-list')
      const deviceListBatch = batchData || {}
      ...
      const sessionBatchData = await migrateIndexKey(parsedKeys, 'session')
      const sessionBatch = sessionBatchData || {}
      ...
      return txn(async () => {
        ...
        for (const { jid } of deviceJids) {
          ...
          if (raw) {
            ...
            if (buf) {
              const sess = SessionRecord.deserialize(buf)
     ...
     * Called once on CB:success before any offline messages are processed.
     * One read of the session blob, one remap pass, one write.
     */
    async migrateAllPNSessionsToLID() {
      return txn(async () => {
        ...
        // Collect all PN-addressed session keys (no domain suffix = WAJIDDomains.WHATSAPP = 0)
        const pnAddrs = sessionKeys.filter(addr => {
          ...
          // Only PN sessions (domain 0 or hosted domain 2)
          return domainType === WAJIDDomains.WHATSAPP || domainType === WAJIDDomains.HOSTED
        })

        if (!pnAddrs.length) return 0

        // Extract unique PN users and batch-lookup their LID mappings
        const pnUserSet = new Set()
        for (const addr of pnAddrs) {
          const [deviceId] = addr.split('.')
          const [user] = deviceId.split('_')
          pnUserSet.add(user)
        }

        const pnUsers = [...pnUserSet]
        const stored = await parsedKeys.get('lid-mapping', pnUsers)

        // Build PN user → LID user map from DB results
        const pnToLidMap = new Map()
        for (const pnUser of pnUsers) {
          const lidUser = stored[pnUser]
          if (lidUser && typeof lidUser === 'string') {
            pnToLidMap.set(pnUser, lidUser)
          }
        }

        if (!pnToLidMap.size) return 0
        ...
        for (const addr of pnAddrs) {
          ...
          const domainType = parseInt(dt || '0')
          const lidUser = pnToLidMap.get(user)
          if (!lidUser) continue

          // Compute the LID address
          ...
          // Skip if LID session already exists
          if (updated[lidAddr]) continue

          const raw = updated[addr]
          if (!raw) continue

          // Copy session data to LID address, delete PN address
          updated[lidAddr] = raw
          delete updated[addr]
          migrated++
        ...
        if (migrated > 0) {
          ...
          logger?.info?.({ migrated, totalPN: pnAddrs.length, mappingsAvailable: pnToLidMap.size }, '[Signal] Batch-migrated PN sessions to LID')
        }
...
function signalStorage({ creds, keys }, lidMapping, logger) {
  ...
  const getIndex = async () => migrateIndexKey(keys, 'session')
