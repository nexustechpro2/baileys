...
import { aesDecryptCTR, aesEncryptGCM, cleanMessage, Curve, decodeMediaRetryNode, decodeMessageNode, makeMutex, decryptMessageNode, delay, derivePairingCodeKey, encodeBigEndian, encodeSignedDeviceIdentity, extractAddressingContext, extractE2ESessionFromRetryReceipt, getCallStatusFromNode, getHistoryMsg, getNextPreKeys, getStatusFromReceiptType, extractMessageText, handleIdentityChange, hkdf, MISSING_KEYS_ERROR_TEXT, NACK_REASONS, NO_MESSAGE_FOUND_ERROR_TEXT, SERVER_ERROR_CODES, toNumber, unixTimestampSeconds, xmppPreKey, xmppSignedPreKey, ACCOUNT_RESTRICTED_TEXT, buildMergedTcTokenIndexWrite, isTcTokenExpired, readTcTokenIndex, resolveTcTokenJid, resolveIssuanceJid, storeTcTokensFromIqResult, TC_TOKEN_INDEX_KEY, decryptMsmsgBotMessage, decodeDecryptedMsmsgMessage, extractReportingInfoFromStanza, getContentType } from "../Utils/index.js"
...
export const makeMessagesRecvSocket = (config) => {
  ...
  const { ev, authState, ws, processingMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager, registerSocketEndHandler, issuePrivacyTokens, fetchAccountReachoutTimelock, placeholderResendCache: sockPlaceholderResendCache } = sock

  const getLIDForPN = signalRepository.lidMapping.getLIDForPN.bind(signalRepository.lidMapping)
  const { messageMutex, notificationMutex, receiptMutex: _receiptMutex, placeholderResendCache } = sock
  ...
  const requestContactRefresh = async (jids) => {
    if (!jids?.length) return
    const pdoMessage = {
      contactRefreshRequest: { jids: Array.isArray(jids) ? jids : [jids] },
      peerDataOperationRequestType: proto.Message.PeerDataOperationRequestType.CONTACT_REFRESH_REQUEST,
    }
    return sendPeerDataOperationMessage(pdoMessage)
  }
  ...
  const handleMessage = async (node) => {
    ...
    if (encNode && encNode.attrs.type === "msmsg") {
      const encIv = getBinaryNodeChildBuffer(encNode, 'enc_iv')
      const encPayload = getBinaryNodeChildBuffer(encNode, 'enc_p') || encNode.content
      if (!encIv || !encPayload) {
        logger.debug({ id: node.attrs.id, from: node.attrs.from, hasIv: !!encIv, hasPayload: !!encPayload }, 'msmsg missing enc_iv or enc_p, cannot decrypt')
        await sendMessageAck(node)
        return
      } else {
        try {
          const msMsg = { encPayload, encIv }
          const messageKey = { participant: node.attrs.participant || node.attrs.from, meId: authState.creds.me.id, meLid: authState.creds.me.lid, conversationJid: node.attrs.from, senderJid: node.attrs.participant, stanzaId: node.attrs.id, targetId: node.attrs.id }
          const messageSecret = authState.creds.myAppStateKeyId ? Buffer.from(authState.creds.myAppStateKeyId, 'base64') : undefined
          if (!messageSecret) { logger.debug({ key: node.attrs.key }, 'no message secret for msmsg, skipping'); await sendMessageAck(node, NACK_REASONS.MissingMessageSecret); return }
          const decrypted = await decryptMsmsgBotMessage(messageSecret, messageKey, msMsg)
          const decoded = decodeDecryptedMsmsgMessage(decrypted)
          logger.debug({ key: node.attrs.key }, 'decrypted msmsg bot message')
          node.attrs._msmsgDecrypted = decoded
        } catch (err) {
          logger.warn({ err, id: node.attrs.id }, 'failed to decrypt msmsg, falling back to nack')
    ...
    try {
      await messageMutex.mutex(async () => {
        await decrypt()
        if (msg.key?.remoteJid && msg.key?.id && msg.message && messageRetryManager) { messageRetryManager.addRecentMessage(msg.key.remoteJid, msg.key.id, msg.message) }
        if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== "peer") {
          if (msg?.messageStubParameters?.[0] === MISSING_KEYS_ERROR_TEXT) { acked = true; return sendMessageAck(node, NACK_REASONS.ParsingError) }
          if (msg.messageStubParameters?.[0] === NO_MESSAGE_FOUND_ERROR_TEXT) {
            const unavailableNode = getBinaryNodeChild(node, "unavailable")
            const unavailableType = unavailableNode?.attrs?.type
            if (unavailableType === "bot_unavailable_fanout" || unavailableType === "hosted_unavailable_fanout" || unavailableType === "view_once_unavailable_fanout") { logger.debug({ msgId: msg.key.id, unavailableType }, "skipping placeholder resend for excluded unavailable type"); acked = true; return sendMessageAck(node) }
            const messageAge = unixTimestampSeconds() - toNumber(msg.messageTimestamp)
            if (messageAge > PLACEHOLDER_MAX_AGE_SECONDS) { logger.debug({ msgId: msg.key.id, messageAge }, "skipping placeholder resend for old message"); acked = true; return sendMessageAck(node) }
            ...
            acked = true; await sendMessageAck(node)
          } else {
            if ((msg.messageStubParameters?.[0] || '').includes('InvalidPreKeyId')) { logger.debug({ msgId: msg.key.id, remoteJid: msg.key.remoteJid }, 'skipping retry for InvalidPreKeyId, sender must re-establish session'); acked = true; return sendMessageAck(node, NACK_REASONS.UnhandledError) }
            if (isJidStatusBroadcast(msg.key.remoteJid)) {
              const messageAge = unixTimestampSeconds() - toNumber(msg.messageTimestamp)
              if (messageAge > STATUS_EXPIRY_SECONDS) { logger.debug({ msgId: msg.key.id, messageAge, remoteJid: msg.key.remoteJid }, "skipping retry for expired status message"); acked = true; return sendMessageAck(node) }
            }
            retryMutex.mutex(async () => {
              ...
              acked = true; await sendMessageAck(node, NACK_REASONS.UnhandledError)
            })
          }
        } else {
          ...
          if (!isNewsletter) {
            ...
            acked = true; await sendReceipt(msg.key.remoteJid, participant, [msg.key.id], type)
        ...
        if (!msg.key.fromMe) {
          try {
            const info = extractReportingInfoFromStanza(node)
            if (info) {
              const senderJid = msg.key.participant || author || msg.key.remoteJid
              const contentType = msg.message ? getContentType(msg.message) : undefined
              config.reportingInfoStore.add(senderJid, {
            ...
            logger.warn({ err: storeErr, key: msg.key }, 'failed to store reporting_tag')
          }
        }
  ...
  async function pruneExpiredTcTokens() {
    try {
      ...
      await authState.keys.set({
        tctoken: {
          ...writes,
          [TC_TOKEN_INDEX_KEY]: { token: Buffer.from(JSON.stringify([...survivors])) }
        }
      })
  ...
  return { ...sock, sendMessageAck, sendRetryRequest, rejectCall, offerCall, fetchMessageHistory, requestPlaceholderResend, requestContactRefresh, messageRetryManager, tcTokenIndexLoaded }
}