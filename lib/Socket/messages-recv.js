...
export const makeMessagesRecvSocket = (config) => {
  ...
  const handleMessage = async (node) => {
    ...
    if (encNode && encNode.attrs.type === "msmsg") {
      try {
        const msMsg = { encPayload: getBinaryNodeChildBuffer(encNode, 'enc_p') || encNode.content, encIv: getBinaryNodeChildBuffer(encNode, 'enc_iv') }
        const messageKey = { participant: node.attrs.participant || node.attrs.from, meId: authState.creds.me.id, meLid: authState.creds.me.lid, conversationJid: node.attrs.from, stanzaId: node.attrs.id, targetId: node.attrs.id }
        ...
        logger.warn({ err, key: node.attrs.key }, 'failed to decrypt msmsg, falling back to nack')
