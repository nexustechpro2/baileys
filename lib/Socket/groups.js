...
export const makeGroupsSocket = (config) => {
    ...
    const aiGroupQuery = (jid, type, content) => query({ tag: 'iq', attrs: { type, xmlns: 'w:g2', to: jid }, content })
    const aiGroupMetadata = async jid => extractAIGroupMetadata(await aiGroupQuery(jid, 'get', [{ tag: 'query', attrs: { request: 'interactive' } }]))
    ...
    return {
        ...
        aiGroupCreate: async (subject, participants = [], options = {}) => {
            const key = generateMessageIDV2()
            const { ephemeralExpiration = 86400 } = options
            return extractAIGroupMetadata(await aiGroupQuery('@g.us', 'set', [{ tag: 'create', attrs: { subject, key }, content: participants.map(jid => ({ tag: 'participant', attrs: { jid } })) }]))
        },
        aiGroupAddBot: async (jid, botUser = '867051314767696') => {
            const result = await aiGroupQuery(jid, 'set', [{ tag: 'add', attrs: {}, content: [{ tag: 'participant', attrs: { jid: `${botUser}@bot` } }] }])
        ...
        aiGroupLeave: async id => aiGroupQuery('@g.us', 'set', [{ tag: 'leave', attrs: {}, content: [{ tag: 'group', attrs: { id } }] }]),
        aiGroupParticipantsUpdate: async (jid, participants, action) => {
            const result = await aiGroupQuery(jid, 'set', [{ tag: action, attrs: {}, content: participants.map(jid => ({ tag: 'participant', attrs: { jid } })) }])
            const node = getBinaryNodeChild(result, action)
            return getBinaryNodeChildren(node, 'participant').map(p => ({ status: p.attrs.error || '200', jid: p.attrs.jid }))
        },
        aiGroupUpdateSubject: async (jid, subject) => aiGroupQuery(jid, 'set', [{ tag: 'subject', attrs: {}, content: Buffer.from(subject, 'utf-8') }]),
        aiGroupInviteCode: async jid => getBinaryNodeChild(await aiGroupQuery(jid, 'get', [{ tag: 'invite', attrs: {} }]), 'invite')?.attrs.code,
        aiGroupRevokeInvite: async jid => getBinaryNodeChild(await aiGroupQuery(jid, 'set', [{ tag: 'invite', attrs: {} }]), 'invite')?.attrs.code,
        aiGroupAcceptInvite: async code => getBinaryNodeChild(await aiGroupQuery('@g.us', 'set', [{ tag: 'invite', attrs: { code } }]), 'group')?.attrs.jid,
        aiGroupSettingUpdate: async (jid, setting) => aiGroupQuery(jid, 'set', [{ tag: setting, attrs: {} }]),
        aiGroupToggleEphemeral: async (jid, ephemeralExpiration) => aiGroupQuery(jid, 'set', [ephemeralExpiration ? { tag: 'ephemeral', attrs: { expiration: ephemeralExpiration.toString() } } : { tag: 'not_ephemeral', attrs: {} }]),
    };
};
...
export const extractAIGroupMetadata = result => {
    ...
    return {
        ...
        owner_country_code: group.attrs.creator_country_code,
        size: group.attrs.size ? +group.attrs.size : getBinaryNodeChildren(group, 'participant').length,
        desc, descId, descOwner, descOwnerPn, descTime,
        isAIGroup: true,
        addressingMode: group.attrs.addressing_mode === 'lid' ? WAMessageAddressingMode.LID : WAMessageAddressingMode.PN,
        participants: getBinaryNodeChildren(group, 'participant').map(({ attrs }) => ({
            id: attrs.jid,
