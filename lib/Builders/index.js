...
import { generateWAMessageFromContent, generateWAMessage, generateMessageIDV2, prepareStickerPackMessage, prepareWAMessageMedia, getUrlInfo } from '../Utils/index.js'
import { getBizBinaryNode } from '../WABinary/index.js'

export { AIRich, aiRichFromObject, buildBotMessageWrapper } from './AIRich.js'
...
export class StickerPack {
  ...
  from(data) {
    const s = Array.isArray(data) ? { stickers: data } : (data && typeof data === 'object' && !data.stickers && Object.keys(data).every(k => !isNaN(k))) ? { stickers: Object.values(data) } : data
    this.#data = s; return this
  }
  ...
  async send(jid, opts = {}) {
    ...
    const result = await prepareStickerPackMessage(this.#data, {
      upload,
      logger: this.#client.logger,
      mediaCache: this.#client.config?.mediaCache,
      options: this.#client.config?.options,
      mediaUploadTimeoutMs: this.#client.config?.mediaUploadTimeoutMs,
      ...passOpts
    })
    if (result.isBatched) {
      let last
...
// ─── Standalone handlers (used by nexus-handler, exported so it stays clean) ──
